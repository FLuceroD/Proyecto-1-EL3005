%-------------------------------------------------------------------------%
% PARTE D) RESPUESTA SEGUN EL ANCHO DE LA SEÑAL
% Se utilizará una funcion parecida a la funcion para la parte C), esta vez
% dependiente de la varianza en lugar de la ganancia.
%-------------------------------------------------------------------------%
...
for i = 1:largo
    if mod(i,2) == 0
        sigmas2(i) = 1/(2*10^(i/2-1));
    else
       sigmas2(i) = 1/(10^((i-1)/2));
    end
end
...
for i = 1:largo
    errores(i) = error_respuesta_senal(k,sigmas2(i));
end
...
titulo=title('Indicador R-S según varianza de la señal emitida');
